...
def provide_feedback(predicted_class, feature_feedback):
    ...
    for feature, coefficient in feature_feedback.items():
        if feature == "Articulation Rate":
            if coefficient > 0:
                feedback_text += f"{feature}:\nYour speaking speed is quite fast, which helps make your words clear and precise. Consider maintaining or refining this aspect.\n\n"
            elif coefficient < 0:
                feedback_text += f"{feature}:\nYou speak a bit slowly. Try to speak more clearly and accurately to improve how you sound..\n\n"
            else:
                feedback_text += f"{feature}:\nYour speaking speed is just right, neither too fast nor too slow..\n\n"
            ...
            if coefficient > 0:
                feedback_text += f"{feature}:\nYour voice pitch is higher than usual.. Maintain or refine your pitch variation for engaging speech.\n\n"
            elif coefficient < 0:
                feedback_text += f"{feature}:\nYour voice pitch is lower than average.. Consider adding more pitch variation for expressive speech.\n\n"
            else:
                feedback_text += f"{feature}:\nYour voice pitch is at a normal level..\n\n"

        elif feature == "Speech Rate":
            if coefficient > 0:
                feedback_text += f"{feature}:\nYou talk quite quickly. Keep up or fine-tune your current speed of speaking..\n\n"
            elif coefficient < 0:
                feedback_text += f"{feature}:\nYou speak a bit slowly. Think about speaking a bit faster to keep your listeners interested.\n\n"
            else:
                feedback_text += f"{feature}:\nYour speed of speaking is just right, neither too fast nor too slow.\n\n"
        ...
        if uploaded_file is not None:
            ...
            with st.spinner('Analyzing audio...'):
                ...
                try:
                    ...
                    for feature, coefficient in feature_values.items():
                        st.write(f"{feature}: {coefficient:.2f}")

                    # Make predictions and generate feedback
                    feedback_text = provide_feedback(predicted_class, feature_values)
                    st.markdown(f"**Feedback:**\n{feedback_text}\n")

                    if predicted_class[0].lower() == 'naive' or predicted_class[0].lower() == 'intermediate':
                    # Assuming example_audio.wav is in the current working directory or provide the full path
                        example_audio_file = 'sneha.wav'

                        # Debug: Check if the file exists
                        if os.path.exists(example_audio_file):
                            st.markdown("**Example Audio:**\nListen to this audio to help improve your speech skills.")
                            st.audio(example_audio_file, format="audio/wav", start_time=0)
                        else:
                            st.error(f"Error: The example audio file '{example_audio_file}' does not exist in the specified path.")

                    else:
                        st.write("No example audio for this class.")
